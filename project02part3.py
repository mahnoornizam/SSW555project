...
def us15_check_lesserthan_15siblings(file_name):
    nodes = parse_nodes(file_name)
    for node in nodes:
        if node.val == 'FAM':
            count = 0
            for first_level_meta in node.meta_data:
                if first_level_meta.key == 'CHIL':
                    count+=1
            print(count)
            assert count < 15

def us16_male_last_names(file_name):
    nodes = parse_nodes(file_name)
    ls = []
    v = None
    for node in nodes:
        for first_level_meta in node.meta_data:
            if first_level_meta.key == 'NAME':
                for k in first_level_meta.meta_data:
                    if k.key == 'SURN':
                        v = k.val
            if first_level_meta.key == 'SEX' and first_level_meta.val == 'M':
                if v:
                    ls.append(v)
        v = None
    print(ls)

us15_check_lesserthan_15siblings('Family.ged')
us16_male_last_names('Family.ged')